...
/**
 * 실무 사용 사례
 * 전화번호 입력 받을 때 - 없어야 하면 걸러내야하기 때문에 '010-1234-5678'이라는 String값(number라면 010이 불가능합니다)에 .indexOf('-')를 확인하여 -1이 return되면 다음 단계로, 3번지가 출력된다면 '-'을 제거하는 메서드로 넘길 수 있습니다.
 */
...
// python에서는 마이너스인덱스가 있기는 한데, JS에서는 없습니다. 하지만 slice()의 argument로 마이너스값을 넣어줄 수는 있습니다.
console.log(strFruits.slice(-12)); //결과값 : Banana, Kiwi
/*
시작점이 -12번지이고 그 다음이 -11 -10 ... -1 (즉 끝)까지 추출하는 메서드였습니다.
근데 -1 하니까 i가 짤리는데 나중에 처리....
 */
console.log(strFruits.substring(7,13));
/*
  그러면 substring() /slice()가 동일한 기능을 하는 것 같아 보이는데, substring()은 음수값을 허용하지 않습니다.

  */
 console.log(strFruits.substr(7,6)); //결과값 : Banana
 /**
  * 얘는 7번지부터 6개의 문자 뽑아내겠다는 겁니다. B부터 시작해서 anana하면 전체 6개의 문자
  */

let welcomeSentence = 'Pleasw visit Seoul and Seoul';
let modifiedSentence = welcomeSentence.replace('Seoul', 'Jeju');
console.log(modifiedSentence); //결과값 : Pleasw visit Seoul and Jeju
/*
 argument와 일치하는 문자열 중 처음 발견된 것을 두 번째 argument로 대체함
*/
let modifiedSentence2 = welcomeSentence.replace('SEOUL', 'Jeju');
console.log(modifiedSentence2); //결과값:Pleasw visit Seoul and Seoul
let modifiedSentence3 = welcomeSentence.replace(/SEOUL/i, 'Jeju');
console.log(modifiedSentence3); //결과값:Pleasw visit Jeju and Seoul
// SEOUL은 찾고자하는 문자열 i는 insensitive의 약자로 덜까다롭게 굴겠다는 뜻
let modifiedSentence4 = welcomeSentence.replace(/Seoul/g, 'Jeju');
console.log(modifiedSentence4); // 결과값 : Pleasw visit Jeju and Jeju
...
/**
 * 한국에서는 성이랑 이름 붙여서 쓰니까 별 문제가 없는데 영어권 애들은 이름 + 미들네임 + 성 이런식으로 공백이 포함됭 있습니다.
 */
...
let noSpaceName = `                    Janme Moca Doe                                `
